    ...
    builder
        .setup(|app| {
            ...
            let profit_menu = CheckMenuItemBuilder::with_id("ticker_profit", "行情条显示盈亏（中键切换）")
                .checked(commands::window::profit_visible(&db)).build(app)?;
            app.manage(commands::window::TickerProfitMenu(profit_menu.clone()));
            ...
            let _tray = TrayIconBuilder::new()
                ...
                .on_menu_event({
                    ...
                    match event.id().as_ref() {
                        "ticker_profit" => {
                            let visible = !commands::window::profit_visible(&db);
                            if let Err(e) = commands::window::apply_profit_visibility(app, &db, visible, false) {
            ...
            if let Some(ticker) = app.get_webview_window("ticker") {
                ...
                let visible = commands::window::profit_visible(&db);
                let _ = ticker.set_size(tauri::LogicalSize::new(if visible { 260 } else { 186 }, commands::window::ticker_height(rows) + if commands::window::market_visible(&db) { 15 } else { 0 }));
                ...
                let _ = commands::window::apply_profit_visibility(app.handle(), &db, visible, true);
