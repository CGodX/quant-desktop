...
mod tests {
    ...
    #[test]
    fn profit_width_preserves_selected_edge_and_height() {
        ...
        for x in [-1800, -350] {
            ...
            let (_, taller) = resize_bounds(hidden, (18, 10), 1.25, 4, ResizeEdge::Top, area);
            assert_eq!(taller.width, hidden.width, "height changes must not reveal/widen the profit column");
        }
    }

    #[test]
    fn exact_midpoint_retains_previous_anchor() {
        let area = Bounds { x: 0, y: 0, width: 1920, height: 1040 };
        let center = Bounds { x: 830, y: 800, width: 260, height: 53 };
        assert!(!anchor_right(center, area, false));
        assert!(anchor_right(center, area, true));
    }
